...
from Utils.ExtractEmptySpots import detections_in_area, handle_horizontal_vertical_area
...
class Tests(unittest.TestCase):
    ...
    def test_scene8_test3(self):
        ### THE PROBLEM: avg car bigger than distance of leftest and second car
        scene_path = os.path.join(self.base_dir, "scene8")
        test_path = os.path.join(scene_path, "test3")
        ious = self.internal_test_code(scene_path, test_path)
        for iou in ious:
    ...
    def find_empty_spots_test(self, image_path, detections, masks, parking_areas):
        free_spots = []
        free_areas = []
    ...
    def internal_test_code(self, scene_path, test_path):
        if os.path.isdir(test_path):
            ...
            if xml_file:
                ...
                test_boxes, test_areas = self.find_empty_spots_test(png_file, detections, masks, parking_areas)
